      ...
      <div className="bg-background min-h-screen">
          ...
          <div className="mx-auto flex h-14 max-w-2xl items-center justify-between gap-3 px-4">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="font-display truncate text-sm font-semibold tracking-wide uppercase">{orgName}</p>
        ...
        <main className="mx-auto max-w-2xl space-y-6 px-4 py-8">
          <div className="border-b border-border pb-4">
            <h1 className="font-display text-3xl font-semibold tracking-wide uppercase">Hola, {greetingName}</h1>
                ...
                <Card className="gap-3 py-5">
                  ...
                  <CardContent className="space-y-3 px-5">
                    {dashboard.memberships.map((m) => (
                      <div key={m.id} className="flex items-baseline justify-between gap-3 border-b border-border pb-3 last:border-0 last:pb-0">
                        <p className="font-medium">{m.plan_name}</p>
                        <p className="text-muted-foreground font-listing text-sm tabular-nums">
                          {m.appointments_remaining}/{m.appointments_total} · {formatAppDate(m.expires_at)}
                        </p>
                      </div>
                    ))}
                  </CardContent>
    ...
    <div className="bg-background flex min-h-screen flex-col">
      <header className="bg-sidebar text-sidebar-foreground px-4 py-8">
        <div className="mx-auto flex max-w-md flex-col items-start gap-3">
          ...
          <div>
            <h1 className="font-display text-3xl font-semibold tracking-wide uppercase">Portal cliente</h1>
