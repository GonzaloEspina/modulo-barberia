-- Portal clients may hold only one future appointment.
-- Staff (admin/barber) keep creating multiple turns from the panel.

CREATE OR REPLACE FUNCTION private.client_has_upcoming_appointment(
  p_client_id UUID,
  p_org_id UUID
)
RETURNS BOOLEAN
...
CREATE OR REPLACE FUNCTION private.assert_portal_one_upcoming(
  p_client_id UUID,
  p_org_id UUID
)
RETURNS VOID
...
AS $$
BEGIN
  IF private.client_has_upcoming_appointment(p_client_id, p_org_id) THEN
    RAISE EXCEPTION 'Ya tenés un turno reservado. Si necesitás otro, pedilo en el local.';
  END IF;
END;
$$;
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('portal-one-upcoming:' || v_client_id::text));
  PERFORM pg_advisory_xact_lock(hashtext(p_barber_id::text || v_date::text));
  PERFORM private.assert_portal_one_upcoming(v_client_id, v_org_id);
...
BEGIN
  ...
  v_has_upcoming := private.client_has_upcoming_appointment(v_client_id, v_org_id);
  ...
  SELECT jsonb_build_object(
    ...
    'has_upcoming_appointment', v_has_upcoming,
